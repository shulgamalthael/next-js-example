...
const HomePage = () => {
    ...
    const handleSetTab = useCallback((tab: string): void => {
        setTab(tab);
        localStorage.setItem('gridTab', JSON.stringify(tab));
    }, []);

    useEffect(() => {
        setTab(JSON.parse(localStorage.getItem('gridTab') || "null") || "grid");
    }, []);
                    ...
                    <div className="w-full">
                        <div className="flex justify-between pl-[30px] w-full mb-[20px]">
                            ...
                            <div className="flex">
                                ...
                                <div onClick={() => handleSetTab('horizontal')} className="flex h-[40px] w-[40px] text-center cursor-pointer">
                                    <img src="/icons/groupsIcon.svg" alt="groupsIcon.svg" className={`max-h-full max-w-full h-auto m-auto border-0 ${tab !== 'horizontal' && 'filter-[invert(46%)_sepia(8%)_saturate(37%)_hue-rotate(316deg)_brightness(97%)_contrast(82%)]'} hover:filter-[brightness(80%)_saturate(0)]`} />
                                </div>
                                <div onClick={() => handleSetTab('grid')} className="flex h-[40px] w-[40px] text-center cursor-pointer">
                                    <img src="/icons/tilesIcon.svg" alt="tilesIcon.svg" className={`max-h-full max-w-full h-auto m-auto border-0 ${tab !== 'grid' && 'filter-[invert(46%)_sepia(8%)_saturate(37%)_hue-rotate(316deg)_brightness(97%)_contrast(82%)]'} hover:filter-[brightness(80%)_saturate(0)]`}/>
                                </div>
                            </div>
                        </div>
                        {tab === 'grid' && <GridCardsList cardsList={cardsList}/>}
                        {tab === 'horizontal' && <HorizontalCardsList cardsList={cardsList} />}
